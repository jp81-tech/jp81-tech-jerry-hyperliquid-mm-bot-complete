...
if [ "$CNT" -gt "$ACTIVE_MAX" ]; then
  head -n "$ACTIVE_MAX" runtime/.merged > runtime/.effective_raw
elif [ "$CNT" -lt "$ACTIVE_MIN" ]; then
  cat runtime/.merged runtime/.rot_top10 | awk "NF" | awk "!seen[\$0]++" | head -n "$ACTIVE_MIN" > runtime/.effective_raw
else
  cp runtime/.merged runtime/.effective_raw
fi

# ═══════════════════════════════════════════════════════════════
# 🤖 AUTO-OPTIMIZER: Filter out trash tokens with sub-$10 notional
# ═══════════════════════════════════════════════════════════════
echo "[AUTO-OPT] Running auto-optimizer filter..."
cat runtime/.effective_raw | bash scripts/auto_optimizer_filter.sh > runtime/.effective
echo "[AUTO-OPT] Filter complete"
