...
if [ -z "${SLACK_WEBHOOK:-}" ] || [ "$SLACK_WEBHOOK" = "CHANGE_ME_SLACK_WEBHOOK_URL" ]; then
  echo "⚠️  SLACK_WEBHOOK not configured, skipping dashboard"
  exit 0
fi
...
DAILY_PNL=$(journalctl -u mm-bot.service --no-pager -n 1000 2>/dev/null | grep -oP 'Daily PnL: [+-]?\$?\K[0-9.-]+' | tail -1 || echo "N/A")

# Get position count
POSITION_COUNT=$(npx tsx scripts/check_positions.ts 2>/dev/null | grep -cE '^\s+[A-Z]+:' || echo "0")

# Get active pairs from logs
ACTIVE_PAIRS=$(journalctl -u mm-bot.service --no-pager -n 50 2>/dev/null | grep -oP '📊 Active pairs \(\d+/\d+\): \K.*' | tail -1 || echo "N/A")
...
FUNDING=$(journalctl -u mm-bot.service --no-pager -n 500 2>/dev/null | grep -oP 'Funding.*\$\K[0-9.-]+' | tail -1 || echo "N/A")
...
# ─────────────────────────────────────────────────────────
MESSAGE="━━━━━━━━━━━━━━━━━━━━━━━━━━
📊 *Hyperliquid MM Bot - Daily Summary*
━━━━━━━━━━━━━━━━━━━━━━━━━━
...
🔥 *Funding (est):* \$$FUNDING/day
🕐 *Nansen Bias Age:* $BIAS_AGE $BIAS_WARNING

━━━━━━━━━━━━━━━━━━━━━━━━━━
Last update: $(date -u +"%Y-%m-%d %H:%M UTC")"

send_slack "$MESSAGE"
echo "✅ Slack dashboard sent"
