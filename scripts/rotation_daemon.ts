...
function runOnce() {
  ...
  const protectedPairs = cfg['protected'] || []
  const finalPairs = [...protectedPairs, ...selected].filter((v, i, a) => a.indexOf(v) === i)
  
  const out = { pairs: finalPairs, ranked, ts: new Date().toISOString() }
  fs.writeFileSync(OUT_PATH, JSON.stringify(out, null, 2))
  console.log(`rotation_evt=selected pairs=${finalPairs.join(',')} topN=${TOP_N} protected=${protectedPairs.join(',')}`)
}
