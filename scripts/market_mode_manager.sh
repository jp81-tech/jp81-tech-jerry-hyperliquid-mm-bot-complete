...
STATE_FILE="runtime/market_mode_state.json"
...
if [ -f "reports/rotator_stats.json" ]; then
  ...
  RED_CNT=$(jq '[.[] | select(.price_change_1h < 0)] | length' reports/rotator_stats.json 2>/dev/null || echo 0)
  ...
  RED_RATIO=$(echo "scale=2; $RED_CNT * 100 / $TOTAL_CNT" | bc 2>/dev/null || echo 0)
fi

# ALO level
ALO_CNT=$(pm2 logs hyperliquid-mm --lines 200 --nostream 2>&1 | grep -Ei "post only|would have immediately" | wc -l | tr -d " ")

# Inventory utilization (simple heuristic)
INV_UTIL=0
if npx tsx scripts/check_positions.ts 2>&1 | grep -q "HYPE\|FARTCOIN"; then
  # If we have positions, estimate 30-50% util as baseline
  INV_UTIL=40
fi

# Decision logic - DUMP has priority over BOUNCE
NEW_MODE="$CURRENT_MODE"

# SEVERE DUMP (highest priority)
if (( $(echo "$BTC_CHANGE < -4.0" | bc -l 2>/dev/null || echo 0) )) && \
   (( $(echo "$VOL_SURGE > 2.5" | bc -l 2>/dev/null || echo 0) )) && \
   (( $(echo "$RED_RATIO > 75" | bc -l 2>/dev/null || echo 0) )); then
  NEW_MODE="SHELTER"

# MODERATE DUMP
elif (( $(echo "$BTC_CHANGE < -2.5" | bc -l 2>/dev/null || echo 0) )) && \
     (( $(echo "$VOL_SURGE > 1.8" | bc -l 2>/dev/null || echo 0) )) && \
     (( $(echo "$RED_RATIO > 60" | bc -l 2>/dev/null || echo 0) )) || \
     (( ALO_CNT > 40 )); then
  NEW_MODE="DEFENSIVE"

# AGGRESSIVE BOUNCE (only if not in dump)
elif [[ "$CURRENT_MODE" != "SHELTER" ]] && [[ "$CURRENT_MODE" != "DEFENSIVE" ]] && \
     (( $(echo "$BTC_CHANGE > 4.0" | bc -l 2>/dev/null || echo 0) )) && \
     (( $(echo "$VOL_SURGE > 2.5" | bc -l 2>/dev/null || echo 0) )) && \
...
# MODERATE BOUNCE
     ...
     (( $(echo "$BTC_CHANGE > 2.5" | bc -l 2>/dev/null || echo 0) )) && \
     (( $(echo "$VOL_SURGE > 1.8" | bc -l 2>/dev/null || echo 0) )) && \
     (( $(echo "$GREEN_RATIO > 60" | bc -l 2>/dev/null || echo 0) )) && \
     (( ALO_CNT < 20 )); then
  NEW_MODE="MODERATE_BOUNCE"

# RETURN TO STABLE - gradual recovery
elif [[ "$CURRENT_MODE" == "SHELTER" ]] && \
     (( $(echo "$BTC_CHANGE > -2.0" | bc -l 2>/dev/null || echo 0) )) && \
     (( $(echo "$RED_RATIO < 50" | bc -l 2>/dev/null || echo 0) )); then
  NEW_MODE="DEFENSIVE"

elif [[ "$CURRENT_MODE" == "DEFENSIVE" ]] && \
     (( $(echo "$BTC_CHANGE > -1.0" | bc -l 2>/dev/null || echo 0) )) && \
     (( $(echo "$VOL_SURGE < 1.5" | bc -l 2>/dev/null || echo 0) )) && \
     (( ALO_CNT < 30 )); then
  NEW_MODE="STABLE"
...
if [ "$NEW_MODE" != "$CURRENT_MODE" ]; then
  ...
  case "$NEW_MODE" in
    SHELTER)
      sed -i "s/^MIN_L1_SPREAD_BPS=.*/MIN_L1_SPREAD_BPS=12/" .env
      sed -i "s/^LAYER_OFFSETS_BPS=.*/LAYER_OFFSETS_BPS=70,110,160,220,290/" .env
      sed -i "s/^CLIP_USD=.*/CLIP_USD=15/" .env
      sed -i "s/^INV_SKEW_K=.*/INV_SKEW_K=0.5/" .env
      sed -i "s/^DRIFT_SKEW_BPS=.*/DRIFT_SKEW_BPS=2/" .env
      MSG="🛡️ MARKET MODE: SHELTER (severe dump protection)
BTC:${BTC_CHANGE}% Vol:${VOL_SURGE}x Red:${RED_RATIO}% ALO:${ALO_CNT}
Applied: Wide spreads 12bps, far offsets, clip 15, max reduction 0.5"
      ;;
    
    DEFENSIVE)
      sed -i "s/^MIN_L1_SPREAD_BPS=.*/MIN_L1_SPREAD_BPS=10/" .env
      sed -i "s/^LAYER_OFFSETS_BPS=.*/LAYER_OFFSETS_BPS=60,95,140,195,260/" .env
      sed -i "s/^CLIP_USD=.*/CLIP_USD=20/" .env
      sed -i "s/^INV_SKEW_K=.*/INV_SKEW_K=0.7/" .env
      sed -i "s/^DRIFT_SKEW_BPS=.*/DRIFT_SKEW_BPS=3/" .env
      MSG="🔰 MARKET MODE: DEFENSIVE (moderate dump)
BTC:${BTC_CHANGE}% Vol:${VOL_SURGE}x Red:${RED_RATIO}% ALO:${ALO_CNT}
Applied: Spreads 10bps, wider offsets, clip 20, reduction 0.7"
      ;;
    ...
    AGGRESSIVE_BOUNCE)
      ...
      MSG="🚀 MARKET MODE: AGGRESSIVE_BOUNCE
BTC:${BTC_CHANGE}% Vol:${VOL_SURGE}x Green:${GREEN_RATIO}% ALO:${ALO_CNT}
Applied: Tight spreads 5bps, close offsets, clip 40, long bias 1.4, 5 layers"
      ;;
    ...
    MODERATE_BOUNCE)
      ...
      MSG="📈 MARKET MODE: MODERATE_BOUNCE
BTC:${BTC_CHANGE}% Vol:${VOL_SURGE}x Green:${GREEN_RATIO}% ALO:${ALO_CNT}
Applied: Spreads 6bps, moderate offsets, clip 30, long bias 1.1"
    ...
    STABLE)
      ...
      MSG="⚪ MARKET MODE: STABLE (normal conditions)
BTC:${BTC_CHANGE}% Vol:${VOL_SURGE}x
Applied: Balanced spreads 8bps, standard offsets, clip 25, neutral 0.9"
      ;;
  esac
  ...
  jq ".mode = \"$NEW_MODE\" | .since = \"$TIMESTAMP\" | .changes += [{\"from\":\"$CURRENT_MODE\",\"to\":\"$NEW_MODE\",\"ts\":\"$TIMESTAMP\",\"btc\":$BTC_CHANGE,\"vol\":$VOL_SURGE,\"green\":$GREEN_RATIO,\"red\":$RED_RATIO,\"alo\":$ALO_CNT}]" "$STATE_FILE" > "$STATE_FILE.tmp"
  ...
  echo "$TIMESTAMP: $CURRENT_MODE -> $NEW_MODE" >> runtime/market_mode_changes.log
fi

echo "mode_check ts=$(date -u +%Y-%m-%dT%H:%M:%SZ) mode=$NEW_MODE btc=$BTC_CHANGE vol=$VOL_SURGE green=$GREEN_RATIO red=$RED_RATIO alo=$ALO_CNT"
