...
# Get positions with better formatting
POS_RAW=$(npx tsx scripts/check_position_pnl.ts 2>/dev/null | egrep -E "^(ZEC|UNI|TOTAL)" || true)

# Format positions into cleaner table
POS=$(echo "$POS_RAW" | awk '
BEGIN {
  printf "Pair  Side   Size      Entry    uPnL\n"
  printf "────────────────────────────────────────\n"
}
{
  pair=$1
  ...
  printf "%-5s %-6s %9.2f  $%-7.2f  %s\n", pair, side, size, entry, upnl
}
' || echo "No positions")
...
# FIXED: Count both buy and sell margin errors
ZEC_MARGIN_ERR=$(printf "%s\n" "$JLOG" | grep -cE "Error placing order \[ZEC (buy|sell)\]:.*Insufficient margin" || true)
UNI_MARGIN_ERR=$(printf "%s\n" "$JLOG" | grep -cE "Error placing order \[UNI (buy|sell)\]:.*Insufficient margin" || true)

# Count fills from "Synced X new fills" format
FILLS_COUNT=$(printf "%s\n" "$JLOG" | grep -oE "Synced [0-9]+ new fills" | awk "{sum+=\$2} END{print sum+0}")
