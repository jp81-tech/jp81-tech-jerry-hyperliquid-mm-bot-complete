...
class LiveTrading implements TradingInterface {
  ...
  async placeOrder(
    ...
    try {
      ...
      console.log(`🔍 DEBUG submit: pair=${pair} size=${finalSizeStr}(${numSizeSteps}steps) price=${finalPriceStr}(${numPriceTicks}ticks) side=${side}`)
  ...
  async mainLoop() {
    while (true) {
      try {
        ...
        // Get ALL open positions from account (including legacy positions)
        const allPositionPairs = await this.getAllPositionPairs()

        // Combine: active pairs + legacy pairs (positions not in top 3)
        const allPairsToTrade = Array.from(new Set([...activePairs, ...allPositionPairs]))
